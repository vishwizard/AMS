...
const RoomCard = ({ room, updatable = false, deletable = false, selectable = false, onSelect = () => { }, onUnSelect = () => { }, updateForm = () => { }, cb }) => {
  ...
  return (
<div className={`max-h-48 border max-w-44 p-4 rounded shadow hover:shadow-lg transition ${selected ? 'border-darkaccent text-gray-400' : (!room?.isBooked ? 'border-green-400' : 'border-red-600')}`}>
